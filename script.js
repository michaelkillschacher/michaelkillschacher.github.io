...
batteryImage.addEventListener('mouseover', () => {
  animateBatteryImages();
});

batteryImage.addEventListener('mouseout', () => {
  resetBatteryImage();
});

function animateBatteryImages() {
  let currentImageIndex = 0;
  const maxImages = 38; // Ändern Sie die Anzahl der Bilder hier

  function changeImage() {
    const nextImageIndex = (currentImageIndex + 1) % (maxImages + 1);
    const paddedIndex = String(nextImageIndex).padStart(4, '0');

    batteryImage.src = `images/battery_connection/battery.${paddedIndex}.png`;

    if (nextImageIndex !== maxImages) {
