...
function animateBatteryImages() {
  ...
  function changeImage() {
    const paddedIndex = String(currentImageIndex).padStart(4, '0');
    ...
    if (currentImageIndex < maxImages) {
      ...
      setTimeout(() => {
        resetBatteryImage();
        isAnimating = false;
      }, 2000); // Warten Sie 2 Sekunden, bevor Sie den Zyklus wieder starten
    }
  }
...
// Mobile Ansicht
batteryImage.addEventListener('click', handleImageInteraction);

// Desktop Ansicht
batteryImage.addEventListener('mouseover', handleImageInteraction);
