...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var activateItem = function (index) {
    ...
    let hoveredMedia = item.querySelector('.media.hovered');
    hoveredMedia.style.backgroundColor = 'rgb(0, 85, 95)';
    hoveredMedia.style.borderColor = 'rgb(250, 100, 0)'; // Setzen Sie die Border Color entsprechend
  ...
  var activateItem = function (index) {
    ...
    var pElement = item.querySelector('.media.hovered .media-body p');
    var dateElement = item.querySelector('.timeline-date');
    var hoveredMedia = item.querySelector('.media.hovered');
    
    // Speichern der ursprünglichen Border Color
    originalBorderColor = window.getComputedStyle(hoveredMedia).getPropertyValue('border-color');
  ...
  var deactivateItem = function (index) {
    ...
    hoveredMedia.style.backgroundColor = '';
    hoveredMedia.style.borderColor = originalBorderColor; // Wiederherstellen der ursprünglichen Border Color
    pElement.style.color = '';
...
batteryImage.addEventListener('touchmove', handleBatteryImageInteraction);

kipperImage.addEventListener('click', handleKipperImageInteraction);
kipperImage.addEventListener('touchmove', handleKipperImageInteraction);

antriebImage.addEventListener('click', handleAntriebImageInteraction);
antriebImage.addEventListener('touchmove', handleAntriebImageInteraction);

automatImage.addEventListener('click', handleAutomatImageInteraction);
automatImage.addEventListener('touchmove', handleAutomatImageInteraction);
